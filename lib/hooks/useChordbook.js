...
export const useChordbook = (chordbookData) => {
  ...
  const reorderChordbook = (chords, affectedRange, directionOfDrag, result) => {
    for (const chord of chords) {
      console.log(
        affectedRange.includes(chord.position),
        chord.position,
        affectedRange
      );
      ...
      //   return chord;
      //   if (inRange(chord.id, affectedRange)) {
      //     chord.position += directionOfDrag;
      //   } else if (chord.id === parseInt(result.destination.droppableId)) {
      //     chord.position = result.destination.index;
      //   }
      // }
    }

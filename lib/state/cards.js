import { FcGoogle, RiSoundModuleLine } from "react-icons";
export const cards = [
  {
    id: 0,
    header: "Dial in a track",
    body: "Search Spotify's library -> select a song -> start a chordbook",
    media: "search.mp4",
  },
  {
    id: 1,
    header: "Drag & Drop chords",
    body: "Use suggested scale chord cards to save progressions",
    media: "drag & drop.mp4",
  },
  {
    id: 2,
    header: "Switch it up",
    body: "Not sounding accurate? Try a different scale, change chord roots or types",
    media: "root.mp4",
  },
  {
    id: 3,
    header: "Sign in w/ Google",
    body: "Authenticate to save chordbooks and bring your profile to any device",
    media: "profile.png",
  },
];

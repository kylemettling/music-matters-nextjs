...
// import chordbook from "../pages/api/graphql/chordbook/chordbook";
...
export function Chordbook({
	...
	// const [currentBooks, setCurrentBooks] = useState(loadedBooks);
	console.log('loaded in Chordbook.js', loadedBooks)
	const {
		chordbooks,
		createStartingBook,
		updateChordbook,
	...
	const [updateDbChordbook, { data, loading, error }] = useMutation(
		...
		// { refetchQueries: [{ query: GET_USER_TRACK }] }
	)
	// 	 {
	// 	variables: { songId: trackId, userId: user?.id, chordbooks },
	// 	skip: !updateTrack,
	// })
	...
	const onDragEnd = useCallback((result) => {
		...
		console.log(
			`onDragEnd start dest:${JSON.stringify(
				destination
			)} source:${JSON.stringify(source)} droppableId:${droppableId}`
		)
		const elements = document.querySelectorAll(
	...
	return (
		<DragDropContext onDragStart={onDragStart} onDragEnd={onDragEnd}>
			{/* <pre>Loaded:{JSON.stringify(chordbooks)}</pre>  */}
			{chordbooks &&
